...
This difference only matters with viewing pictures in telega.  PDFs
seem to be opaque.
***** Font
:PROPERTIES:
...
:END:
The keys already work correctly, but Emacs also receives them and complains.
#+BEGIN_SRC emacs-lisp
  (if is-kitchensink (progn
...
:END:
This keys already work correctly , but Emacs also receives them and complains.
#+BEGIN_SRC emacs-lisp
  (if is-kitchensink (progn
...
#+BEGIN_SRC emacs-lisp
  ...
  ;                                       #'async-shell-command)))
#+END_SRC
*** Center cursor on frame movement
#+BEGIN_SRC emacs-lisp
  (when is-kitchensink
    (udh/install-and-load 'exwm-mff)
    (setq exwm-mff-mode t
