...
def count_sequence_in_repo(pat, exclude="^[#]", file_types=["h", "c", "hpp", "cpp"]):
    ...
    result = subprocess.run(
        ["git", "count--pat-exclude-types", pat, exclude]
        + ["*." + t for t in file_types],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True,
    )
    if result.returncode != 0:
        raise RuntimeError(f"Error listing files in repository: {result.stderr}")
...
def evolve_string_in_repo(regex, min_generations=100, pop_size=100, mutation_rate=0.1):
    ...
    with tqdm(total=pop_size, desc="Initial Population ") as pb:
        ...
        while len(population) < pop_size:
            candidate = "".join(
                random.choices(string.printable[:-4], k=random.randint(3, 6))
            )
            s = fitness(candidate)
            if s == 0:
    ...
    best_strings = []
    best_count = max(2, min(pop_size // 10, int(pop_size ** (0.5))))
    ...
    while generation < generations:
        ...
        print(
            f"Generation {generation + 1}/{generations} {generation_percent}%:",
            best_strings[:5],
        )
        if generation != 0:
...
if __name__ == "__main__":
    ...
    top_strings = evolve_string_in_repo(
        regex, min_generations=100, pop_size=100, mutation_rate=0.2
    )
    print("\nFinal Top Strings:")
